...
import { StreambinClient, UploadableFile, UploadableFileStream } from "@streambin/sdk";
...
type StreambinConfig = {
  selectedBucketSlug?: string;
...
const CONFIG_DIR = path.join(homedir(), ".streambin");
const CONFIG_FILE = path.join(CONFIG_DIR, "config.json");
const STATE_FILE = path.join(CONFIG_DIR, "state.json");
const DEFAULT_BASE_URL = process.env.STREAMBIN_BASE_URL ?? "https://streambin.xyz";
...
async function readConfig(): Promise<StreambinConfig> {
  return readJsonFile<StreambinConfig>(CONFIG_FILE, { buckets: {} });
}

async function readState(): Promise<StreambinState> {
  return readJsonFile<StreambinState>(STATE_FILE, { cursors: {} });
}

async function saveState(state: StreambinState) {
...
function generateSlug(existing: Set<string>): string {
...
function parseBucketImportToken(token: string): { slug: string; namespace: string; passphrase: string } {
  const trimmed = token.trim();
  const payload = trimmed.startsWith("npx ")
    ? trimmed.slice(trimmed.lastIndexOf("import ") + "import ".length)
    : trimmed;
...
function stateKey(namespace: string, streamPath: string): string {
  return `${namespace}:${streamPath}`;
}

function getSelectedBucket(config: StreambinConfig): BucketConfig {
...
function clientForBucket(bucket: BucketConfig): StreambinClient {
...
async function useBucket(slugInput: string) {
  const config = await readConfig();
...
async function exportBucket(slugInput?: string) {
  const config = await readConfig();
...
async function importBucket(token: string, baseUrl: string) {
  const config = await readConfig();
...
function wireBucketCommands(commandRoot: Command) {
  commandRoot
    .command("create")
    .argument("[slug]")
    .option("--name <slug>", "Bucket slug (same as [slug])")
    .option("--namespace <namespace>", "Custom namespace")
    .option("--passphrase <passphrase>", "Custom passphrase")
    .option("--base-url <url>", "API base URL", DEFAULT_BASE_URL)
    .action(
      async (
        inputSlug: string | undefined,
        opts: { name?: string; namespace?: string; passphrase?: string; baseUrl: string },
      ) => {
        const slugFromFlag = opts.name ? normalizeSlug(opts.name) : undefined;
        const slugFromArg = inputSlug ? normalizeSlug(inputSlug) : undefined;
  ...
  commandRoot
    ...
    .argument("<token>")
    .option("--base-url <url>", "API base URL", DEFAULT_BASE_URL)
    .action(async (token: string, opts: { baseUrl: string }) => {
...
program
  .name("streambin.xyz")
  .description("Encrypted stream buckets for agents")
  .showHelpAfterError();
...
program
  ...
  .argument("<stream>")
  .argument("<message>")
  .action(async (stream: string, message: string) => {
...
program
  ...
  .option("--mode <mode>", "message|json|both", "both")
  .option("--interval <ms>", "poll interval in ms", "2000")
  .action(async (stream: string, opts: { from?: string; mode: "message" | "json" | "both"; interval: string }) => {
    const config = await readConfig();
    const bucket = getSelectedBucket(config);
    const client = clientForBucket(bucket);
...
program
  .command("get")
  .argument("<stream>")
  .option("--after <timestamp>")
  .option("--limit <n>", "max 100", "100")
  .option("--mode <mode>", "message|json|both", "both")
  .action(async (stream: string, opts: { after?: string; limit: string; mode: "message" | "json" | "both" }) => {
    const config = await readConfig();
    const bucket = getSelectedBucket(config);
    const client = clientForBucket(bucket);
    const state = await readState();
...
program
  ...
  .argument("<targetPath>")
  .argument("<json>")
  .action(async (targetPath: string, json: string) => {
    const bucket = getSelectedBucket(await readConfig());
    const client = clientForBucket(bucket);
...
program
  ...
  .action(async (targetPath: string, json: string) => {
    ...
    const updated = await client.updateObject<Record<string, unknown>>(targetPath, patch);
...
program
  ...
  .argument("<targetPath>")
  .argument("<timestamp>")
  .action(async (targetPath: string, timestamp: string) => {
    const bucket = getSelectedBucket(await readConfig());
    const client = clientForBucket(bucket);
...
function buildStreamingSource(filePath: string, size: number, contentType?: string): UploadableFileStream {
  return {
    size,
    contentType,
    stream() {
...
program
  .command("upload-file")
  .description("Upload a local file to a namespaced path (public, expires in 3 days, encrypted by default)")
  .argument("<targetPath>", "Path within the bucket namespace (e.g. assets/logo.png)")
  .argument("<filePath>", "Local file path to upload")
  .option("--content-type <type>", "Override the inferred Content-Type")
  .option("--no-encrypt", "Upload as plaintext (default: encrypt with bucket passphrase)")
  .option("--chunk-size <bytes>", "Encrypted chunk size in bytes (default: 8 MiB)")
  .option("--multipart", "Force multipart upload")
  .option("--no-multipart", "Disable multipart upload")
  .option("--part-size <bytes>", "Multipart part size in bytes (min 5MB, plaintext mode only)")
  .action(
    async (
      targetPath: string,
      filePath: string,
      ...
      const multipart =
        typeof opts.multipart === "boolean" || opts.partSize
          ? { enabled: opts.multipart, partSize: opts.partSize ? Number(opts.partSize) : undefined }
          : undefined;
...
program
  .command("file-url")
  .description("Print the stable Streambin URL that redirects to the file's public URL (serves ciphertext for encrypted files)")
  .argument("<targetPath>")
  .action(async (targetPath: string) => {
    const bucket = getSelectedBucket(await readConfig());
    const client = clientForBucket(bucket);
...
program
  ...
  .description("Download a file (auto-decrypts encrypted files using the bucket passphrase)")
  .argument("<targetPath>")
  .argument("<destination>", "Local destination file path")
  .action(async (targetPath: string, destination: string) => {
    const bucket = getSelectedBucket(await readConfig());
    const client = clientForBucket(bucket);
    ...
    const nodeStream = Readable.fromWeb(
      downloaded.stream as import("node:stream/web").ReadableStream<Uint8Array>,
    );
...
program
  .command("remove-file")
  .description("Delete a file from the bucket (metadata + S3 object)")
  .argument("<targetPath>")
  .action(async (targetPath: string) => {
    const bucket = getSelectedBucket(await readConfig());
    const client = clientForBucket(bucket);
